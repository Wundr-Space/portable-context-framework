...
export async function queryContext(input) {
    ...
    if (input.tags && input.tags.length > 0) {
        filtered = filtered.filter((session) => {
            return input.tags.every(tag => session.tags.some(t => t.toLowerCase() === tag.toLowerCase()));
        });
    }
    ...
    // Limit results
    const results = sorted.slice(0, limit).map((session) => ({
        id: session.id,
