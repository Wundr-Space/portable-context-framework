...
import { CONTEXT_DATA_DIR, ROOT_DIR, loadAllSessions, parseRelativeDate } from '../lib/storage.js';
...
function buildUserStory(session) {
...
function buildWikiDocument(plan) {
    const header = [
...
async function persistPlan(plan, wikiContent) {
...
export async function generateBacklog(input) {
    ...
    const features = filteredSessions.map(buildFeature);
    const plan = {
        project: input.project,
...
//# sourceMappingURL=generate-backlog.js.map