 ...
 */
export async function loadAllSessions(project) {
    const sessions = [];
    try {
 ...
 */
export function getSessionFilePath(session) {
    return path.join('context-data', 'projects', session.project, 'sessions', `${session.timestamp}-${session.id}.json`);
}
/**
